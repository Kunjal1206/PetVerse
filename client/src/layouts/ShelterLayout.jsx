...
const ShelterLayout = () => {
  ...
  const navItems = [
    { name: 'Shelter Overview', path: '/shelter/dashboard', icon: LayoutDashboard, end: true },
    { name: 'Manage Pets', path: '/shelter/pets', icon: PawPrint },
    { name: 'Add New Pet', path: '/shelter/pets/add', icon: PlusCircle },
    { name: 'Adoption Applications', path: '/shelter/applications', icon: Calendar },
  ];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <aside className="lg:col-span-1">
            <div className="bg-white rounded-3xl border border-cream-300 p-5 shadow-premium sticky top-28 space-y-6">
              ...
              {/* Shelter Profile Summary */}
              <div className="flex items-center gap-3 pb-5 border-b border-cream-200">
                <div className="w-12 h-12 rounded-2xl bg-brand-accent text-white font-display font-bold text-lg flex items-center justify-center shadow-xs">
                  <Building2 className="w-6 h-6" />
                </div>
                <div className="min-w-0">
                  <div className="flex items-center gap-1.5">
                    <h3 className="font-display font-bold text-base text-charcoal-900 truncate">
                      {shelter?.name || user?.name || 'Shelter'}
                    </h3>
                    <ShieldCheck className="w-4 h-4 text-emerald-600 shrink-0" title="Verified Shelter" />
                  </div>
                  <p className="text-xs text-charcoal-500 truncate">{shelter?.city || user?.location || 'India'}</p>
                  <span className="inline-block mt-0.5 px-2 py-0.2 text-[10px] font-bold uppercase rounded bg-teal-50 text-teal-800 border border-teal-200">
                    Rescue Partner
                  </span>
                </div>
              </div>
